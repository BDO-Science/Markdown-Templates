...
date: "`r format(Sys.Date(), '%B %d, %Y')`" # Automatically inserts the current date in "Month Day, Year" format
...
```{r}
# Example R code to calculate the mean of a vector
...
x <- seq(-10, 10, by = 0.1)
y <- x^2
plot(x, y, type = "l", main = "Example Plot", xlab = "X-axis", ylab = "Y-axis")
```
...
kable(example_table, format = "pipe")
```
